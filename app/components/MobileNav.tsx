    ...
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
        <div className="absolute left-0 top-0 bottom-0 w-3/4 max-w-xs p-4 shadow-lg overflow-auto animate-slide-in" style={{ background: 'var(--surface)' }}>
          ...
          <Link href="/login" onClick={onClose} className="px-4 py-2 rounded hover:bg-gray-100 dark:hover:bg-white/5" style={{ color: 'var(--text)' }}>Giriş Yap</Link>
          <Link href="/register" onClick={onClose} className="px-4 py-2 rounded text-center transition-colors" style={{ background: 'var(--accent)', color: 'var(--bg)' }}>Kayıt Ol</Link>
        </nav>
